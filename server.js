...
function auth(req, res, next) {
  ...
  catch { res.clearCookie('token'); return res.status(403).json({ error: 'Invalid session' }); }
}

function resolveUrl(url, type) {
  let u = url || '';
  if (type === 'archive') { var m = u.match(/archive\.org\/details\/([^/?\s]+)/); if (m) u = 'https://archive.org/download/' + m[1] + '/' + m[1] + '.mp4'; }
  if (type === 'ipfs' && u.startsWith('ipfs://')) u = u.replace('ipfs://', 'https://gateway.pinata.cloud/ipfs/');
  return u;
}
function extractYoutubeId(url) { if (!url) return null; var m = url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/); return m ? m[1] : null; }
function sanitize(s) { if (!s) return ''; return String(s).replace(/[<>'";&]/g, '').substring(0, 1000); }

async function setupAdmin() {
  if (!supabase) return;
  try { var { data } = await supabase.from('admin_users').select('id').limit(1); if (!data?.length) { var h = await bcrypt.hash('Admin@2024', 14); var pin = crypto.randomBytes(4).toString('hex'); await supabase.from('admin_users').insert({ username: 'admin', password_hash: h, pin: pin }); console.log('Admin: admin / Admin@2024 / PIN: ' + pin); } } catch (e) { console.error('Admin setup:', e.message); }
}
setupAdmin();

/* ═══ AUTH ═══ */
app.post('/api/v1/auth/check-pin', pinLimiter, async (req, res) => {
  try { var { username, pin } = req.body; if (!username || !pin || pin.length !== 8) return res.json({ valid: false }); var { data } = await supabase.from('admin_users').select('pin').eq('username', sanitize(username)).single(); res.json({ valid: data?.pin === pin }); } catch { res.json({ valid: false }); }
});
app.post('/api/v1/auth/login', authLimiter, async (req, res) => {
  try {
    ...
    res.cookie('token', token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 8 * 60 * 60 * 1000, path: '/' }).json({ username: admin.username, success: true });
  } catch { res.status(500).json({ error: 'Server error' }); }
});
app.post('/api/v1/auth/logout', (req, res) => { res.clearCookie('token', { path: '/' }).json({ success: true }); });
...
app.post('/api/v1/audio/request-stream/:audioId', streamLimiter, async (req, res) => {
  try {
    ...
    const streamToken = jwt.sign({ audioId: a.id, type: a.audio_type || 'mp3', media: 'audio', ip: req.ip, uid: crypto.randomBytes(6).toString('hex') }, STREAM_SECRET, { expiresIn: '300s' });
...
app.post('/api/v1/posts/:id/view', streamLimiter, async (req, res) => { try { var { data: p } = await supabase.from('posts').select('views').eq('id', req.params.id).single(); if (!p) return res.status(404); await supabase.from('posts').update({ views: (p.views || 0) + 1 }).eq('id', req.params.id); res.json({ success: true }); } catch { res.status(500); } });
app.get('/api/v1/audio', async (req, res) => { try { var { data } = await supabase.from('audio_tracks').select('id,title,artist,cover_url,duration,category,views,created_at').eq('is_published', true).order('created_at', { ascending: false }); res.json(data || []); } catch { res.json([]); } });
app.get('/api/v1/settings', async (req, res) => { try { var { data } = await supabase.from('settings').select('*'); var s = {}; (data || []).forEach(i => s[i.key] = i.value); res.json(s); } catch { res.json({}); } });
app.post('/api/v1/contacts', contactLimiter, async (req, res) => { try { var { alias_name, contact_method, message_type, message } = req.body; if (!alias_name || !message) return res.status(400); var { data, error } = await supabase.from('contacts').insert({ alias_name: sanitize(alias_name).substring(0,50), contact_method: sanitize(contact_method||'').substring(0,100), message_type, message: String(message).substring(0,2000) }).select().single(); if (error) throw error; res.json({ success: true, id: data.id }); } catch { res.status(500); } });
...
app.post('/api/v1/admin/categories', auth, async (req, res) => { try { var d = req.body; if (!d.name) return res.status(400); var { data, error } = await supabase.from('categories').insert({ name: sanitize(d.name), description: d.description||'', icon: d.icon||'fa-folder', order_index: d.order_index||0 }).select().single(); if (error) throw error; res.json(data); } catch { res.status(500); } });
...
app.put('/api/v1/admin/settings', auth, async (req, res) => { try { for (var [k, v] of Object.entries(req.body)) { if (k.length > 50) continue; await supabase.from('settings').upsert({ key: k, value: typeof v === 'object' ? JSON.stringify(v) : String(v).substring(0,5000), updated_at: new Date().toISOString() }); } res.json({ success: true }); } catch { res.status(500); } });
app.put('/api/v1/admin/credentials', auth, async (req, res) => {
  try {
    ...
    if (new_password) { if (new_password.length < 8) return res.status(400).json({ error: 'Password 8+ chars' }); u.password_hash = await bcrypt.hash(new_password, 14); }
    if (new_pin) { if (new_pin.length !== 8) return res.status(400).json({ error: 'PIN 8 digits' }); u.pin = new_pin; }
    ...
    const newToken = jwt.sign({ id: req.admin.id, username: new_username || req.admin.username, ip: req.ip }, ADMIN_JWT_SECRET, { expiresIn: '8h' });
...
app.get('/api/v1/admin/analytics', auth, async (req, res) => {
  try { var { data: videos } = await supabase.from('videos').select('id,title,views,created_at').order('views', { ascending: false }); var { data: contacts } = await supabase.from('contacts').select('id,is_read,created_at'); var totalViews = 0; for (var v of (videos||[])) totalViews += (v.views||0); var unread = 0; for (var c of (contacts||[])) if (!c.is_read) unread++; var thisWeek = 0; var wAgo = new Date(); wAgo.setDate(wAgo.getDate()-7); for (var v2 of (videos||[])) if (new Date(v2.created_at)>=wAgo) thisWeek++; res.json({ totalViews, topVideos: (videos||[]).slice(0,5), totalVideos: (videos||[]).length, unread, thisWeek }); } catch { res.json({ totalViews:0, topVideos:[], totalVideos:0, unread:0, thisWeek:0 }); }
});

app.get('*', (req, res) => { res.sendFile(path.join(__dirname, 'public', 'index.html')); });
app.listen(PORT, () => console.log('Maxaas.u on port ' + PORT));