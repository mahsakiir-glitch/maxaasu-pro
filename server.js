// ============================================
// Maxaas.u Pro - Production Server
// ============================================
...
function generateStreamToken(videoId) {
  return jwt.sign({ videoId: videoId, type: 'stream' }, JWT_SECRET, { expiresIn: '2h' });
}
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      const { error: insertError } = await supabase
        .from('admin_users')
        .insert({ username: 'admin', password_hash: hash, pin: '12345678' });
      if (insertError) {
...
app.get('/api/v1/stream-token/:videoId', async (req, res) => {
  try {
    const { data: video } = await supabase
      .from('videos')
      .select('id, is_published, video_type')
    ...
    res.json({
      token: token,
      streamUrl: '/api/v1/stream/' + video.id + '?token=' + token,
...
app.get('/api/v1/audio-token/:trackId', async (req, res) => {
  try {
    ...
    res.json({ token: token, streamUrl: '/api/v1/audio-stream/' + track.id + '?token=' + token });
  } catch (e) {
    res.status(500).json({ error: 'Failed to generate token' });
  }
});
